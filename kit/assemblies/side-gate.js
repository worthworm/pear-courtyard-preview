...
(function(){
  ...
  function build(sink,spec,style){
    ...
    const eaveOverhang=value.eaveOverhang||.15,eaveDepth=roofDepth+eaveOverhang*2,wingWidth=Math.max(.15,(roofWidth-doorOpeningWidth)*.24),wingHeight=gateHeight-.04,wingDepth=eaveDepth,wingOffset=doorOpeningWidth/2+wingWidth/2-.015,wingZ=0,jambWidth=Math.max(.07,joinThickness*.68),jambDepth=eaveDepth,jambZ=0,jambOffset=doorOpeningWidth/2+jambWidth/2,platformWidth=doorOpeningWidth+.22,platformDepth=.46,platformZ=.12;
    ...
    sink.metadata({id:value.id,type:'sideGate',openingWidth:openingWidth,visualDoorOpeningWidth:doorOpeningWidth,gateHeight:gateHeight,gateDepth:gateDepth,doorLeafCount:2,doorOpenAngle:doorOpenAngle,doorOpenMode:doorOpenMode,equalLeafOpenAngle:true,doorConstruction:'double-vertical-plank',roofType:'small-gable',columnStyle:'slender-octagonal',embeddedWallReveal:true,entryPlatform:true,doorRing:true,openingClear:true,interiorBacking:false,sideWallsAlignedToEaves:true,sideWallDepth:eaveDepth,sideWallWidth:wingWidth,authoringParts:authoringParts,authoringOffsets:authoring,authoringScales:authoringScales,visualBounds:{position:position,width:roofWidth,height:roofBaseY+roofHeight,depth:eaveDepth},gameplayFootprint:null});
