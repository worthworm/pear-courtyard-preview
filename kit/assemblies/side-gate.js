...
(function(){
  ...
  function build(sink,spec,style){
    ...
    const bakedOffsets={leftColumn:[-.0858,0,.0264],rightColumn:[.0858,0,0]};
    const bakedScales={platform:[1.05,1,1]};
    ...
    function scl(id){const raw=authoringScales[id],base=bakedScales[id]||[1,1,1];return base.map(function(value,index){const n=Array.isArray(raw)?Number(raw[index]):1;return value*(Number.isFinite(n)?Math.max(.1,Math.min(4,n)):1);});}
    ...
    const nominalWallThickness=Math.max(.20,joinThickness*1.05),
      openingEdge=doorOpeningWidth/2,
      nominalWallInner=openingEdge+.02,
      wallOuter=nominalWallInner+nominalWallThickness,
      postX=nominalWallInner-postRadius*.22,
      sideWallInner=postX+postRadius+.015,
      sideWallThickness=Math.max(.08,wallOuter-sideWallInner),
    ...
    sink.withTransform({position:position},function(){
      ...
      withPart('door',pivots.door,function(){const halfOffset=(leafWidth+doorGap)/2,frameThickness=style.beamThickness*.34,innerInset=style.beamThickness*.34;JiangnanPlankDoorLeaf.build(sink,{id:value.id+'/door/leaf-left',position:[-halfOffset,doorBottom,doorZ],width:leafWidth,height:doorHeight,depth:doorDepth,frameThickness:frameThickness,innerInset:innerInset,kind:'leaf',openMode:doorOpenMode,hingeSide:'left',openAngle:doorOpenAngle,seed:seed+7,boardCount:5,seamGap:.010,braceCount:0,boardColor:style.doorBoardColor||sink.tone(sink.color('wood'),.11),hasRing:true},style);JiangnanPlankDoorLeaf.build(sink,{id:value.id+'/door/leaf-right',position:[halfOffset,doorBottom,doorZ],width:leafWidth,height:doorHeight,depth:doorDepth,frameThickness:frameThickness,innerInset:innerInset,kind:'leaf',openMode:doorOpenMode,hingeSide:'right',openAngle:doorOpenAngle,seed:seed+8,boardCount:5,seamGap:.010,braceCount:0,boardColor:style.doorBoardColor||sink.tone(sink.color('wood'),.11),hasRing:true},style);});
    });
