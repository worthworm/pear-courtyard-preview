...
(function(){
  ...
  function build(sink,spec,style){
    ...
    const eaveOverhang=value.eaveOverhang||.15,eaveDepth=roofDepth+eaveOverhang*2,eaveFront=roofDepth/2+eaveOverhang,wallFront=eaveFront-.025,wallBack=doorZ-gateDepth*.42,jambWidth=Math.max(.07,joinThickness*.68),outerJoin=openingWidth/2+joinThickness*.92,innerJoin=doorOpeningWidth/2+jambWidth*.25,wingWidth=Math.max(.18,outerJoin-innerJoin),wingOffset=(outerJoin+innerJoin)/2,wingHeight=gateHeight-.04,wingDepth=wallFront-wallBack,wingZ=(wallFront+wallBack)/2,jambFront=wallFront-.01,jambBack=doorZ-gateDepth*.10,jambDepth=jambFront-jambBack,jambZ=(jambFront+jambBack)/2,jambOffset=doorOpeningWidth/2+jambWidth/2,postZ=wallFront-postRadius*1.05,platformWidth=doorOpeningWidth+.20,platformBack=doorZ-gateDepth*.20,platformFront=wallFront+.08,platformDepth=platformFront-platformBack,platformZ=(platformFront+platformBack)/2;
    const pivots={platform:[0,.055,platformZ],leftWall:[-wingOffset,wingHeight*.56,wingZ],rightWall:[wingOffset,wingHeight*.56,wingZ],door:[0,doorBottom+doorHeight*.55,doorZ],roof:[0,roofBaseY+roofHeight*.40,0],leftColumn:[-postX,columnTop*.56,postZ],rightColumn:[postX,columnTop*.56,postZ],frame:[0,roofBaseY+.08,wallFront-.05]};
    sink.withTransform({position:position},function(){
      ...
      withPart('frame',pivots.frame,function(){
        sink.scope('side-gate/lintel','wood',function(){sink.box(0,gateHeight-.11,postZ-.025,doorOpeningWidth+postRadius*3.2,.11,.13,'wood',JiangnanRandom.signed(seed,'lintel')*style.woodVariation*.12);sink.box(0,gateHeight-.18,wallFront-.10,doorOpeningWidth+postRadius*2.8,.055,.08,'wood',-.018);});
        sink.scope('side-gate/gable-frame','wood',function(){
          const frontZ=wallFront-.015,left=[-roofWidth*.33,roofBaseY-.015,frontZ],right=[roofWidth*.33,roofBaseY-.015,frontZ],ridge=[0,roofBaseY+roofHeight*.74,frontZ];
        ...
        sink.scope('side-gate/brackets','wood',function(){[-1,1].forEach(function(side){const x=side*postX;sink.box(x,gateHeight-.20,postZ,.10,.18,.10,'wood',-.02);sink.beam3([x,gateHeight-.16,postZ],[x+side*.15,roofBaseY+.025,wallFront-.03],.038,.038,'wood',-.018);});});
      });
    ...
    const lanternZ=position[2]+eaveFront,lanternY=position[1]+gateHeight-.18;
    ...
    sink.metadata({id:value.id,type:'sideGate',openingWidth:openingWidth,visualDoorOpeningWidth:doorOpeningWidth,gateHeight:gateHeight,gateDepth:gateDepth,doorLeafCount:2,doorOpenAngle:doorOpenAngle,doorOpenMode:doorOpenMode,equalLeafOpenAngle:true,doorConstruction:'double-vertical-plank',roofType:'small-gable',columnStyle:'slender-octagonal',embeddedWallReveal:true,entryPlatform:true,doorRing:true,openingClear:true,interiorBacking:false,sideWallsAlignedToEaves:true,sideWallFront:wallFront,sideWallBack:wallBack,sideWallDepth:wingDepth,sideWallWidth:wingWidth,postZ:postZ,authoringParts:authoringParts,authoringOffsets:authoring,authoringScales:authoringScales,visualBounds:{position:position,width:roofWidth,height:roofBaseY+roofHeight,depth:Math.max(eaveDepth,platformDepth)},gameplayFootprint:null});
