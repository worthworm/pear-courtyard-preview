...
(function(){
  ...
  function build(sink,spec,style){
    ...
    function withPart(id,action){sink.withTransform({position:off(id)},action);}
    ...
    sink.withTransform({position:position},function(){
      withPart('platform',function(){sink.scope('side-gate/entry-platform','stone',function(){sink.box(0,.045,gateDepth*.46+.14,openingWidth+.34,.09,.52,'stone');});});
      withPart('leftWall',function(){sink.scope('side-gate/wing-wall-left','wall',function(){sink.box(-wingOffset,wingHeight/2,wingZ,wingWidth,wingHeight,wingDepth,'wall');sink.box(-jambOffset,(gateHeight-.05)/2,-gateDepth*.05,jambWidth,gateHeight-.05,jambDepth,'wall');});});
      withPart('rightWall',function(){sink.scope('side-gate/wing-wall-right','wall',function(){sink.box(wingOffset,wingHeight/2,wingZ,wingWidth,wingHeight,wingDepth,'wall');sink.box(jambOffset,(gateHeight-.05)/2,-gateDepth*.05,jambWidth,gateHeight-.05,jambDepth,'wall');});});
      withPart('leftColumn',function(){
        sink.scope('side-gate/stone-base-left','stone',function(){sink.box(-postX,baseHeight/2,.015,postRadius*2.55,baseHeight,postRadius*2.85,'stone');});
        sink.scope('side-gate/column-left','wood',function(){octagonalColumn(sink,-postX,.015,postRadius,baseHeight,columnHeight,'wood',JiangnanRandom.signed(seed,'side-gate-post-left')*style.woodVariation*.14);});
      });
      withPart('rightColumn',function(){
        sink.scope('side-gate/stone-base-right','stone',function(){sink.box(postX,baseHeight/2,.015,postRadius*2.55,baseHeight,postRadius*2.85,'stone');});
        sink.scope('side-gate/column-right','wood',function(){octagonalColumn(sink,postX,.015,postRadius,baseHeight,columnHeight,'wood',JiangnanRandom.signed(seed,'side-gate-post-right')*style.woodVariation*.14);});
      });
      withPart('frame',function(){
        ...
        sink.scope('side-gate/gable-frame','wood',function(){const frontZ=roofDepth*.34,left=[-roofWidth*.34,roofBaseY-.015,frontZ],right=[roofWidth*.34,roofBaseY-.015,frontZ],ridge=[0,roofBaseY+roofHeight*.72,frontZ];sink.beam3(left,ridge,.058,.058,'wood',-.012);sink.beam3(ridge,right,.058,.058,'wood',-.012);sink.beam3([0,roofBaseY-.015,frontZ],[0,roofBaseY+roofHeight*.48,frontZ],.046,.046,'wood',-.018);});
      });
      withPart('roof',function(){JiangnanRoof.build(sink,{id:value.id,width:roofWidth,depth:roofDepth,baseY:roofBaseY,roofHeight:roofHeight,eaveOverhang:value.eaveOverhang||.15,mode:(value.roofComplexity||'LITE').toUpperCase(),seed:seed+31,showRafters:true,eaveThicknessScale:.72,ridgeScale:.84},style);});
    });
    const doorOffset=off('door');
    JiangnanPlankDoorLeaf.build(sink,{id:value.id+'/door/leaf-single',position:[position[0]+doorOffset[0],position[1]+doorBottom+doorOffset[1],position[2]+gateDepth*.035+doorOffset[2]],width:doorWidth,height:doorHeight,depth:doorDepth,frameThickness:style.beamThickness*.40,innerInset:style.beamThickness*.40,kind:'leaf',openMode:doorOpenMode,hingeSide:'right',openAngle:doorOpenAngle,seed:seed+7,boardCount:5,seamGap:.014,braceCount:0,boardColor:[.46,.44,.40],hasRing:true},style);
    ...
    const authoringParts=[
      {id:'leftWall',label:'左侧墙',position:world([-wingOffset,wingHeight*.56,wingZ],'leftWall')},
      {id:'rightWall',label:'右侧墙',position:world([wingOffset,wingHeight*.56,wingZ],'rightWall')},
      {id:'door',label:'门扇',position:world([0,doorBottom+doorHeight*.55,gateDepth*.035],'door')},
      {id:'roof',label:'门楼屋顶',position:world([0,roofBaseY+roofHeight*.40,0],'roof')},
      {id:'leftColumn',label:'左柱',position:world([-postX,columnTop*.56,.015],'leftColumn')},
      {id:'rightColumn',label:'右柱',position:world([postX,columnTop*.56,.015],'rightColumn')},
      {id:'frame',label:'门楼木构',position:world([0,roofBaseY+.08,roofDepth*.24],'frame')},
      {id:'platform',label:'门前石台',position:world([0,.08,gateDepth*.46+.14],'platform')}
    ];
    sink.metadata({id:value.id,type:'sideGate',openingWidth:openingWidth,gateHeight:gateHeight,gateDepth:gateDepth,doorLeafCount:1,doorOpenAngle:doorOpenAngle,doorOpenMode:doorOpenMode,doorConstruction:'single-vertical-plank',roofType:'small-gable',columnStyle:'slender-octagonal',embeddedWallReveal:true,entryPlatform:true,doorRing:true,openingClear:true,interiorBacking:false,authoringParts:authoringParts,authoringOffsets:authoring,visualBounds:{position:position,width:roofWidth,height:roofBaseY+roofHeight,depth:roofDepth+(value.eaveOverhang||.15)*2},gameplayFootprint:null});
