...
(function(){
  ...
  function build(sink,spec,style){
    ...
    const wingWidth=Math.max(.30,(roofWidth-doorOpeningWidth)*.48),wingHeight=gateHeight-.04,wingDepth=gateDepth*.88,wingOffset=doorOpeningWidth/2+wingWidth/2-.015,wingZ=-gateDepth*.06,jambWidth=Math.max(.07,joinThickness*.68),jambDepth=gateDepth*.90,jambOffset=doorOpeningWidth/2+jambWidth/2,platformWidth=doorOpeningWidth+.22,platformDepth=.46,platformZ=.12;
    ...
    sink.withTransform({position:position},function(){
      ...
      withPart('leftWall',pivots.leftWall,function(){sink.scope('side-gate/wing-wall-left','wall',function(){sink.box(-wingOffset,wingHeight/2,wingZ,wingWidth,wingHeight,wingDepth,'wall');sink.box(-jambOffset,(gateHeight-.08)/2,doorZ,jambWidth,gateHeight-.08,jambDepth,'wall');});});
      withPart('rightWall',pivots.rightWall,function(){sink.scope('side-gate/wing-wall-right','wall',function(){sink.box(wingOffset,wingHeight/2,wingZ,wingWidth,wingHeight,wingDepth,'wall');sink.box(jambOffset,(gateHeight-.08)/2,doorZ,jambWidth,gateHeight-.08,jambDepth,'wall');});});
      ...
      withPart('roof',pivots.roof,function(){JiangnanRoof.build(sink,{id:value.id,width:roofWidth,depth:roofDepth,baseY:roofBaseY,roofHeight:roofHeight,eaveOverhang:value.eaveOverhang||.15,mode:(value.roofComplexity||'LITE').toUpperCase(),seed:seed+31,showRafters:true,eaveThicknessScale:.70,ridgeScale:.84},style);});
    ...
    sink.metadata({id:value.id,type:'sideGate',openingWidth:openingWidth,visualDoorOpeningWidth:doorOpeningWidth,gateHeight:gateHeight,gateDepth:gateDepth,doorLeafCount:2,doorOpenAngle:doorOpenAngle,doorOpenMode:doorOpenMode,equalLeafOpenAngle:true,doorConstruction:'double-vertical-plank',roofType:'small-gable',columnStyle:'slender-octagonal',embeddedWallReveal:true,entryPlatform:true,doorRing:true,openingClear:true,interiorBacking:false,authoringParts:authoringParts,authoringOffsets:authoring,authoringScales:authoringScales,visualBounds:{position:position,width:roofWidth,height:roofBaseY+roofHeight,depth:roofDepth+(value.eaveOverhang||.15)*2},gameplayFootprint:null});
