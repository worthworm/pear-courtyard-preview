...
(function(){
  ...
  function build(sink,spec,style){
    ...
    const pivots={
      platform:[0,.045,platformZ],
      leftWall:[-sideWallX,sideWallHeight*.55,sideWallZ],
      rightWall:[sideWallX,sideWallHeight*.55,sideWallZ],
      door:[0,doorBottom+doorHeight*.55,doorZ],
      roof:[0,roofBaseY+roofHeight*.40,0],
      leftColumn:[-postX,columnTop*.56,postZ],
      rightColumn:[postX,columnTop*.56,postZ],
      frame:[0,roofBaseY+.09,postZ]
    };
    ...
    function sideGableFrame(x,side){
      const baseY=roofBaseY-.015,
        zRear=-roofHalfDepth+.06,
        zFront=roofHalfDepth-.06,
        ridge=[x,roofBaseY+roofHeight*.90,0],
        rear=[x,baseY,zRear],
        front=[x,baseY,zFront];
      sink.beam3(rear,ridge,.052,.052,'wood',-.016);
      sink.beam3(ridge,front,.052,.052,'wood',-.016);
      sink.beam3([x,baseY+.015,zRear*.76],[x,baseY+.015,zFront*.76],.045,.045,'wood',-.020);
      sink.beam3([x,baseY+.015,0],[x,roofBaseY+roofHeight*.54,0],.040,.040,'wood',-.022);
      sink.box(x,roofBaseY+roofHeight*.43,0,.055,.15,.075,'wood',-.020);
      sink.beam3([x,gateHeight-.14,postZ-.02],[x,roofBaseY+.06,zFront*.78],.040,.040,'wood',-.015);
      if(side<0)sink.box(x-.018,gateHeight-.20,postZ,.070,.16,.090,'wood',-.018);
      else sink.box(x+.018,gateHeight-.20,postZ,.070,.16,.090,'wood',-.018);
    }

    sink.withTransform({position:position},function(){
      withPart('platform',pivots.platform,function(){
        sink.scope('side-gate/entry-platform','stone',function(){sink.box(0,.025,platformZ,platformWidth,.05,platformDepth,'stone');});
      });

      withPart('leftWall',pivots.leftWall,function(){
        sink.scope('side-gate/side-wall-left','wall',function(){
          sink.box(-sideWallX,sideWallHeight/2,sideWallZ,sideWallThickness,sideWallHeight,sideWallDepth,'wall');
          sink.box(-jambX,(gateHeight-.10)/2,jambZ,jambWidth,gateHeight-.10,jambDepth,'wall');
          gableWall(-sideWallX);
        });
      });

      withPart('rightWall',pivots.rightWall,function(){
        sink.scope('side-gate/side-wall-right','wall',function(){
          sink.box(sideWallX,sideWallHeight/2,sideWallZ,sideWallThickness,sideWallHeight,sideWallDepth,'wall');
          sink.box(jambX,(gateHeight-.10)/2,jambZ,jambWidth,gateHeight-.10,jambDepth,'wall');
          gableWall(sideWallX);
        });
      ...
      withPart('leftColumn',pivots.leftColumn,function(){
        sink.scope('side-gate/stone-base-left','stone',function(){sink.box(-postX,baseHeight/2,postZ,postRadius*2.55,baseHeight,postRadius*2.85,'stone');});
        sink.scope('side-gate/column-left','wood',function(){octagonalColumn(sink,-postX,postZ,postRadius,baseHeight,columnHeight,'wood',JiangnanRandom.signed(seed,'side-gate-post-left')*style.woodVariation*.14);});
      });

      withPart('rightColumn',pivots.rightColumn,function(){
        sink.scope('side-gate/stone-base-right','stone',function(){sink.box(postX,baseHeight/2,postZ,postRadius*2.55,baseHeight,postRadius*2.85,'stone');});
        sink.scope('side-gate/column-right','wood',function(){octagonalColumn(sink,postX,postZ,postRadius,baseHeight,columnHeight,'wood',JiangnanRandom.signed(seed,'side-gate-post-right')*style.woodVariation*.14);});
      });

      withPart('frame',pivots.frame,function(){
        sink.scope('side-gate/front-lintel','wood',function(){
          sink.box(0,gateHeight-.10,postZ,postX*2+postRadius*2.2,.12,.13,'wood',JiangnanRandom.signed(seed,'lintel')*style.woodVariation*.12);
          sink.box(0,gateHeight-.19,postZ+.015,postX*2+postRadius*1.7,.055,.075,'wood',-.018);
        });
        sink.scope('side-gate/side-gables','wood',function(){
          sideGableFrame(-sideWallX,-1);sideGableFrame(sideWallX,1);
        });
        sink.scope('side-gate/front-brackets','wood',function(){
          [-1,1].forEach(function(side){const x=side*postX;sink.beam3([x,gateHeight-.16,postZ],[x+side*.12,roofBaseY+.035,postZ+.03],.038,.038,'wood',-.018);});
        });
      });

      withPart('roof',pivots.roof,function(){
        JiangnanRoof.build(sink,{id:value.id,width:roofWidth,depth:roofDepth,baseY:roofBaseY,roofHeight:roofHeight,eaveOverhang:eaveOverhang,mode:(value.roofComplexity||'LITE').toUpperCase(),seed:seed+31,showRafters:true,eaveThicknessScale:.70,ridgeScale:.84},style);
      });
    ...
    const authoringParts=[
      {id:'leftWall',label:'左侧墙',position:world(pivots.leftWall,'leftWall')},
      {id:'rightWall',label:'右侧墙',position:world(pivots.rightWall,'rightWall')},
      {id:'door',label:'双扇门',position:world(pivots.door,'door')},
      {id:'roof',label:'门楼屋顶',position:world(pivots.roof,'roof')},
      {id:'leftColumn',label:'左柱',position:world(pivots.leftColumn,'leftColumn')},
      {id:'rightColumn',label:'右柱',position:world(pivots.rightColumn,'rightColumn')},
      {id:'frame',label:'门楼木构',position:world(pivots.frame,'frame')},
      {id:'platform',label:'门前石台',position:world(pivots.platform,'platform')}
    ];

    sink.metadata({
      id:value.id,type:'sideGate',openingWidth:openingWidth,visualDoorOpeningWidth:doorOpeningWidth,
      gateHeight:gateHeight,gateDepth:gateDepth,doorLeafCount:2,doorOpenAngle:doorOpenAngle,
      doorOpenMode:doorOpenMode,equalLeafOpenAngle:true,doorConstruction:'double-vertical-plank',
      roofType:'small-gable',columnStyle:'slender-octagonal',embeddedWallReveal:true,
