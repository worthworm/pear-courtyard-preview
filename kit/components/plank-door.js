...
(function(){
  function build(sink,spec,style){
    ...
    sink.withTransform({position:spec.position||[0,0,0],pivot:[hingeX,0,0],yaw:(hingeSide==='left'?1:-1)*currentAngle},function(){
      ...
      sink.scope('plank-door/boards','wood',function(){
        for(let i=0;i<boardCount;i++){
          ...
          sink.box(x,height/2,depth*.035,boardWidth,height,depth*.86,boardColor,band);
        }
      });
    ...
    sink.anchor({id:spec.id+'/hinge',position:[(spec.position||[0,0,0])[0]+hingeX,(spec.position||[0,0,0])[1]+height/2,(spec.position||[0,0,0])[2]],normal:[0,0,1],tags:['hinge','runtime-animation']});
    return metadata;
  }
